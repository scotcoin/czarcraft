Running the Czarcraft software:

Dependencies: Java 7 or later needs to be installed first. Only the Oracle JVM
has been tested by TZR team.

There is no installation needed. Unpack the czarcraft-client.zip package and open a
shell in the resulting czarcraft directory. Execute the run.sh script if using Linux,
...
The java process communicates with peers on port 8874 tcp by default. If you are
...
The default Czarcraft client does not store any wallet-type file on disk. Unlike
...
The source is included. It is also available at https://github.com/czarcoin/czarcraft .    
