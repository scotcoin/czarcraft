var NRS = (function(NRS, $, undefined) {
	...
	$("#register_alias_modal").on("show.bs.modal", function(e) {
		...
		if (alias) {
			NRS.sendRequest("getAliasURI", {
				...
				} else if (/acct:(\d+)@nfd/.test(response.uri) || /nacc:(\d+)/.test(response.uri)) {
	...
	NRS.forms.setAliasType = function(type, uri) {
		...
		if (type == "uri") {
			...
			$("#register_alias_help").html("The alias will reference the account number entered and can be used to send NFD to, messages, etc..").show();
		} else {
